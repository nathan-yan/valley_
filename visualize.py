...
fonts = {'calibri':font.SysFont('calibri', 30), 'calibri20':font.SysFont('calibri', 25)}
...
res = pygame.display.list_modes()[2]
...
class Box:
    def __init__(self, text, surface, pos = [0, 0], color = (255, 100, 100), text_color = (0, 0, 0), size = 10, font = 'calibri', node = None, has_params = False, text_boxes_ = []):
        ...
        self.width = self.text_width + 30   # 5 pixels of border on either side 
        self.height = self.text_height + 30 # 5 pixels of border on either side 
        ...
        self.connection_points[Point([self.pos[0] + self.width/2, self.pos[1] - 14])] = None
    ...
    def draw(self, sample = False):
        ...
        screen.blit(self.text, (self.pos[0] + 15, self.pos[1] + 15))
        
        if sample:
            aaCircle(self.surface, self.delete_color, [self.pos[0] + self.width + 2, self.pos[1] - 2], 6)
    
            keys = self.connection_points.keys()
            for p in range (len(keys)):
                pygame.draw.aaline(self.surface, (10, 10, 10), (self.pos[0] + self.width/2, self.pos[1]), keys[p].coordinate)
                if self.connection_points[keys[p]] != None:
                    aaCircle(screen, (150, 50, 50), [keys[p][0], keys[p][1]], 6)
                else:
                    aaCircle(screen, (50, 155, 50), [keys[p][0], keys[p][1]], 6)
    ...
    def isInDelete(self):
        point = pygame.mouse.get_pos()
        return distance(point, [self.pos[0] + self.width + 2, self.pos[1] - 2]) <= 7
    ...
    def reposition(self, pos):
        ...
        for p in range(len(keys)):
            ...
            if keys[p].coordinate == [self.pos[0] + self.width/2, self.pos[1] - 14]:
                keys[p].coordinate = [pos[0] + self.width/2, pos[1] - 14] 
                non_orig = True 
    ...
    def draw(self):
        ...
        if self.is_editing: 
            if self.response_width < 40:
                pygame.draw.rect(screen, (254, 255, 255), (self.pos[0] + self.form_width, self.pos[1] - 2,  50, self.height))
            else:
                pygame.draw.rect(screen, (254, 255, 255), (self.pos[0] + self.form_width, self.pos[1] - 2,  self.response_width + 10, self.height))
            ...
            if sample.isIn():
                ...
                if pygame.mouse.get_pressed()[0] and is_click:
                    if sample.orig_text == "convolution":
                        ...
                        boxes.append(Box(text = "convolution / " + nodes[-1].name, surface = screen, pos = sample.pos, color = sample.default_color, has_params = True, text_boxes_ = meta_text_boxes[0]))
                        print len(nodes) 
                        layers += 1
                        nodes.append(Node(name = 'param' + str(layers), type = 'param', param_size = nodes[-1].param_size))
                        boxes.append(Box(text = 'param / ' + nodes[-1].name, surface = screen, pos = [sample.pos[0] + boxes[-1].width + 2, sample.pos[1]], color = (100, 100, 100)))

                        layers += 1
                    elif sample.orig_text == "matmul":
                        nodes.append(Node(name = 'dot' + str(layers), type = "matmul", param_size = [1, 1]))
                        boxes.append(Box(text = "matmul / " + nodes[-1].name, surface = screen, pos = sample.pos, color = sample.default_color, has_params = True, text_boxes_ = meta_text_boxes[1]))
                        print len(nodes) 
                        layers += 1
                        nodes.append(Node(name = 'param' + str(layers), type = 'param', param_size = nodes[-1].param_size))
                        boxes.append(Box(text = 'param / ' + nodes[-1].name, surface = screen, pos = [sample.pos[0] + boxes[-1].width + 2, sample.pos[1]], color = (100, 100, 100)))
                        ...
                        boxes.append(Box(text = "ReLU / " + nodes[-1].name, surface = screen, pos = sample.pos, color = sample.default_color))
                        print len(nodes) 
                        layers += 1
                    elif sample.orig_text == "sigmoid":
                        nodes.append(Node(name = 'sigmoid' + str(layers), type = "sigmoid"))
                        boxes.append(Box(text = "sigmoid / " + nodes[-1].name, surface = screen, pos = sample.pos, color = sample.default_color))
                        print len(nodes) 
                        layers += 1
                    elif sample.orig_text == "input":
                        nodes.append(Node(name = 'inp' + str(layers), type = "INPUT"))
                        boxes.append(Box(text = "input / " + nodes[-1].name, surface = screen, pos = sample.pos, color = sample.default_color))
                        print len(nodes) 
                        layers += 1
                    elif sample.orig_text == "mse_loss":
                        nodes.append(Node(name = 'loss' + str(layers), type = 'mse_loss'))
                        boxes.append(Box(text = 'mse_loss / ' + nodes[-1].name, surface = screen, pos = sample.pos, color = sample.default_color))
                        layers += 1
                    elif sample.orig_text == "grad":
                        nodes.append(Node(name = 'grad' + str(layers), type = "gradient"))
                        boxes.append(Box(text = 'grad / ' + nodes[-1].name, surface = screen, pos = sample.pos, color = sample.default_color))
                        layers += 1
                    
                    elif sample.orig_text == 'flatten':
                        nodes.append(Node(name = 'flat' + str(layers), type = 'flatten'))
                        boxes.append(Box(text = 'flatten / ' + nodes[-1].name, surface = screen, pos = sample.pos, color = sample.default_color))
                        layers += 1
                    elif sample.orig_text == 'target':
                        nodes.append(Node(name = 'target' + str(layers), type = 'target'))
                        boxes.append(Box(text = 'target / ' + nodes[-1].name, surface = screen, pos = sample.pos, color = sample.default_color))
                        layers += 1
                    elif sample.orig_text == "output":
                        nodes.append(Node(name = 'out' + str(layers), type = 'output'))
                        boxes.append(Box(text = 'output / ' + nodes[-1].name, surface = screen, pos = sample.pos, color = sample.default_color))
                        layers += 1
                    
                    elif sample.orig_text == "merge":
                        nodes.append(Node(name = 'merge' + str(layers), type = 'merge', param_size = [-1]))
                        boxes.append(Box(text = 'merge / ' + nodes[-1].name, surface = screen, pos = sample.pos, color = sample.default_color, text_boxes_ = meta_text_boxes[2]))
        ...
        for box in boxes:
            ...
            if box.is_editing:
                for text in box.text_boxes:
                    ...
                    if text.is_editing == False and is_click:    # We (might have) lost focus of the text box
                        try:
                            idx = boxes.index(box)
            ...
            # print box.connection_points
                ...
                if distance(mouse_pos, box.connection_points.keys()[p].coordinate) < 9 and is_click:
                    is_dragging = [box, p]
                    is_focused = [box, p]
                    is_click = False 
        ...
        if is_dragging != False:
            if type(is_dragging) == list:       # We are dragging a connection point 
                ...
                for p in range(len(is_dragging[0].connection_points.keys())):
                    if is_dragging[0].connection_points.keys()[p].coordinate == [is_dragging[0].pos[0] + is_dragging[0].width/2, is_dragging[0].pos[1] - 14]:
                        non_orig = True 
                
                if not non_orig:
                    is_dragging[0].connection_points[Point([is_dragging[0].pos[0] + is_dragging[0].width/2, is_dragging[0].pos[1] - 14])] = None
